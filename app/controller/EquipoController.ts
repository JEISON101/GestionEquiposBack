import client from "../database/pgDatabase";
...
export default class equiposcontroller{
    ...
    async postEquipo({request,response}){
    ...
    async putEquipo({params, request, response}){
        const id = params.id
        const {nombre,anio_fundacion} = request.body()
    ...
    async deleteEquipo({params,response}:HttpContextContract){
        const id = params.id
